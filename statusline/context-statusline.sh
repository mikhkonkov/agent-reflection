#!/bin/bash
# agent-reflection — statusline context meter for Claude Code.
#
...
#
# Env:
#   AGENT_REFLECTION_STATUSLINE_CHAIN
...
PREFIX=""
if [ -n "$AGENT_REFLECTION_STATUSLINE_CHAIN" ]; then
  PREFIX=$(printf '%s' "$INPUT" | eval "$AGENT_REFLECTION_STATUSLINE_CHAIN" 2>/dev/null \
