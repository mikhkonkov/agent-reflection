...
const SINGLE_PATH_KEYS = ["file_path", "path", "notebook_path"];
/** Keys on a tool_input object whose value is (or may be) an array of path strings. */
const ARRAY_PATH_KEYS = ["paths", "files"];
const MAX_PATHS = 20;
 ...
 */
export function extractRelativePaths(toolInput, repoRoot) {
    ...
    for (const candidate of candidates) {
        ...
        if (result.length >= MAX_PATHS)
            break;
    }
    return result;
}
/** Count of repo-relative paths referenced by a tool_input payload. */
export function countPaths(toolInput, repoRoot) {
