...
export const contextPressureRule = {
    id: RULE_ID,
    evaluate(ctx) {
        ...
        if (!hasAuto && !hasManyTotal && !hasBytePressure)
            return null;
        const severity = hasAuto ? "high" : "warning";
        let confidence = 0.5;
        if (hasAuto)
            confidence += 0.2 + Math.min(0.15, 0.05 * (metrics.autoCompactions - 1));
        if (hasManyTotal)
            confidence += 0.1 + Math.min(0.1, 0.02 * (metrics.totalCompactions - 2));
        if (hasBytePressure) {
        ...
        return {
            ...
            suggestedAction: "Move broad searches, large logs, and external research to isolated subagents, and return only short structured summaries to the main agent.",
            evidence: {
