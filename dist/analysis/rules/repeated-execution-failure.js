...
export const repeatedExecutionFailureRule = {
    ...
    evaluate(ctx) {
        ...
        if (!dominantCategory || dominantCount < 2)
            return null;
        const affectedRelativePaths = Array.from(new Set(executionFailures.flatMap((f) => f.relativePaths))).sort();
        ...
        return {
            ...
            suggestedAction: "After two similar failures, stop extending the same hypothesis and request an independent diagnosis before making further edits.",
            evidence: {
