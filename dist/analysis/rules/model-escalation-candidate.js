...
export const modelEscalationCandidateRule = {
    ...
    evaluate(ctx) {
        ...
        if (outcome === "accepted")
            return null;
        const severity = outcome === "failed" || outcome === "rework" ? "high" : "warning";
        ...
        const confidence = clampConfidence(0.45 + outcomeBonus + 0.02 * turnsOverage + 0.05 * failureOverage);
        return {
            ruleId: RULE_ID,
            severity,
            confidence,
            title: "Model Escalation Candidate",
            rationale: "The task has signals of an unproductive implementation loop or unresolved ambiguity.",
            suggestedAction: "Consider launching `architect-escalation` with a compact handoff: goal, current diff, failing command, error category, relevant files, and attempted hypotheses.",
            evidence: {
