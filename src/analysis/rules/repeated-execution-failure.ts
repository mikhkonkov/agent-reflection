...
export const repeatedExecutionFailureRule: Rule = {
  ...
  evaluate(ctx: RuleContext): Recommendation | null {
    ...
    const affectedRelativePaths = Array.from(
      new Set([...executionFailures.flatMap((f) => f.relativePaths), ...modifiedPaths]),
    ).sort();
    ...
    return {
      ...
      rationale:
        `A run command failed ${times(executionFailures.length)} on ${describeErrorCategory(dominantCategory)}, ` +
        `with ${edits(modificationCountBetweenFailures)}${whereClause} in between — each fix was followed by the same class of failure.`,
      suggestedAction:
        "Stop editing and get an independent diagnosis: hand off the goal, the failing command, its error output, and the hypotheses already tried to `architect-escalation`.",
      evidence: {
